<#
Title: Deploy-CMTrace
Author: Ryan McKenna
Date: 4/5/2023
Description:
This script 'installs' CMTrace on a client computer.
The application does not need to be installed. The exe will run on it's own.
So all that is needed is a simple file transfer.
This is used on Intune-only managed devices where the ConfigMgr client (Software Center) is not installed
like it would be on a co-managed or SCCM-only managed computer.
The most recent copy of this application can be retreived from VMWAS117 at the following location:
C:\Program Files\Microsoft Configuration Manager\cd.latest\SMSSETUP\TOOLS
#>

#See if the directory already exists, if not, then create it
if (-not (Test-Path -Path C:\MSUDenver -PathType Container))
{
    New-Item -Path "C:\" -Name "MSUDenver" -ItemType "directory"
}

#Copy CMTrace.exe into the folder. Will overwrite if already there
Copy-Item -Path .\CMTrace.exe -Destination C:\MSUDenver\

#Intune detection method does the work of verifying if the script worked

# SIG # Begin signature block
# MIImVgYJKoZIhvcNAQcCoIImRzCCJkMCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAkzvoIDXNaqMr+
# d6WU1FCF23SAMocX1S+lWGq5CraKyKCCH8EwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# AYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQg/I//rD3GbcSi
# SMeGf3qeEe091Wz5VAXWCcwGcBpXB6wwDQYJKoZIhvcNAQEBBQAEggGALIFYzGa9
# P9C/mqQboEe0bs9i9iZpYKVPzmHFw7EWMTHOxs9koturl3i96YOQft6QWijiIay4
# uimNcthGV4tMwfrhoYZ7JrEslt8xKOBmBmATnziKKSXyAT7tU4bnbQrSqg3fDDMS
# T8rv3bdUHZ/jCg3TnNH7vfJQT2yiggeiB+wVy4Oacp3fG+g9z4Sy/x7v5wCXy2G6
# Vp12Y3pzLUT7L6v3QlIy9Mw3Y9eeYdhYPOnshOruaFQOdxjaE0U7EJs4RkjU3dQ9
# tF0n96EQOjBBbAnaqDBhIBZgItZifptcJnd+wnK1XWu0DWb9ezZBuRvXHszaiKqv
# a22el9Iq4iSmnPNOCbS0+efqFTl2SFuT7JAKMl9mhdqhWqfFH12eoDcv6oG0LiQj
# 4BMGPawJBpkpDyzWOA2KbQ8whYVvS90vjt6Vx3EzMA6dT3a7EvLO1/11mkShP+Br
# T/vPkpQ8K4IddcQce+li7Lt12x23a6h2cyiTbImtvqA7+TDtx9XlZTM7oYIDTDCC
...
# MTIzMjFaMD8GCSqGSIb3DQEJBDEyBDD3bDH3EHnBbY4/Vu7ZvKjC28tbblXq42tp
# P+GjEw3koFmHpe447Zr3NHv1QOb0oVAwDQYJKoZIhvcNAQEBBQAEggIAT8oakQ4C
# Jhwcvv4N4jBy92gTmQLmap4ca6fjuhknFvvJzxj+MkXlZF5pEKcSs51RWW2mnk+g
# /lpQTz4vPWif3cAoEk5VTk8JCJstemxE5E4o389z/5Dr4Np9t2Z8DE+aD4Lm4UGn
# CSQOW0Na/ih3F8OIJcyLlHFp9c5QqvJPZV/aSHoFHz7r9m9lTubTS4+o3ubqgyss
# h7ioKlYgoIOz1s9YIZtB8oB9Xlq6Tu8DZKeL0+wIGgBMwK88x4kGdQWUk34geZ0z
# WZyC0PaAtKdTaIYoc13jDBxYboQRj05q4crGDRApkQFM/CL3vOKEWM3+zq2mKjjE
# VGGEn76tshE9s2j9+K8TQYCCFgctXHkvrmM+3qULfl7lK35wNRc66VONh9RQzPfr
# onMui9CrKL46gqvqqa+SMF0UUudBuLmVuM1Okn3Xg4mQhIDwyys38Z5xO8Y7r+Yf
# mpmqnbpE9Nk9nV7AdC7CTLcOKh5HbJdkb/GJyQKiI0xTsLH3Vyc5GaINjgm3ginX
# +oeLKQWGSilYye+Fk1N0r7x/XeqOWtyKB8BUjfaEP9vgfqF209xE/fLPApz1xZxV
# /jkUeiSHZ26DOh8Uo2SkSIz39g25A2O1JUWQ/MpOSmW8D2pOoJJCmI1E1xvGUsyz
# yaRD/t61R2Zui1ItJPk1sqfdCt9oq20v2hY=
